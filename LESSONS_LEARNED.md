...
## 10. Authoritative Multiplayer Damage & State Desynchronization
When syncing combat between a Host and multiple Clients, broadcasting only the "base damage" of an attack and allowing each peer to independently calculate the final damage based on their local view of the target's state (e.g., applying a 50% defense modifier if the unit is "entrenched") is a fatal architectural flaw. Minor latency fluctuations mean a unit might have been stationary for 30.1 seconds on the Client (entrenching it) but only 29.9 seconds on the Host.
* **Warning**: If the Host deletes a unit because it mathematically took lethal damage, but the Client halved that same damage locally and kept the unit alive, it yields a permanent "Ghost Unit" that remains visibly engaged in combat indefinitely on the Client but ceases to exist entirely on the Host.
* **Fix**: Damage application must be entirely authoritative. The Host must extract the target's local defense modifiers, compute the exact final hit points to deduct, and broadcast that *final unmodified value* to all peers so all clients forcefully deduct the exact same amount of health indiscriminately.
