...
func test_cruiser_coastal_deployment():
	# Configure mock map data directly
	# For CoastalCity, we force tile 10 to be OCEAN
	# For LandlockedCity, we force tile 20 to be PLAINS
	# the exact lat/lons don't matter as long as they map to separate test tiles
	...
	# Override the terrain types on the mock map data strictly for checking
	globe_view.map_data._terrain_map[coastal_tile] = "OCEAN"
	globe_view.map_data._terrain_map[land_tile] = "PLAINS"
	
	# Test the inner _city_has_water checker manually
	...
	# In a headless environment, simulating an unhandled input click event to perfectly align with a raycast 
	# that hits a mock mesh is extremely brittle. Instead, we can simulate the exact boolean bounds logic
	# from lines 1256-1264 of GlobeView._unhandled_input
	
	globe_view.deploying_unit_type = "Cruiser"
	globe_view.deploying_unit_cost = 50.0
	
	# Simulate checking the "CoastalCity"
	...
	if globe_view.deploying_unit_type == "Cruiser":
	...
	assert_true(is_valid_coastal, "CoastalCity must satisfy all terrain and resource requirements for Cruiser deployment")
	
	# Simulate checking the "LandlockedCity"
	...
	assert_false(is_valid_landlocked, "LandlockedCity must be rejected for Cruiser deployment because valid_terrain resolves to false")
